...
import { reorderQuestionnaireQuestions } from "@/lib/questionnaires/service";
import { parseJsonBody } from "@/lib/validators/clients";
import { reorderQuestionnaireQuestionsSchema } from "@/lib/validators/questionnaires";
...
export const POST = withApiHandler(
  ...
  async ({ request }) => {
    ...
    const body = await parseJsonBody(
      reorderQuestionnaireQuestionsSchema,
      request,
    );
    const questionnaire = await reorderQuestionnaireQuestions(
      org.organizationId,
      questionnaireId,
      body.questionIds,
    );
