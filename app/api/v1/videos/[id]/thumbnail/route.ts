...
import {
  getVideoThumbnailForDownload,
  getVodThumbnailBlobStream,
...
export const GET = withApiHandler({ requireOrg: true }, async ({ request }) => {
  ...
  const pathname = await getVideoThumbnailForDownload(
    org.organizationId,
    videoId,
    actor,
  );

  const blob = await getVodThumbnailBlobStream(pathname);
  if (!blob) {
    throw problem({
      type: "not-found",
      title: "Thumbnail not found",
      status: 404,
      detail: "The thumbnail could not be retrieved.",
    });
  }
